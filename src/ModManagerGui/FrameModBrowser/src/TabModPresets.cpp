  ...
  if(isPreset_){
    ...
    item->registerAction("Remove", brls::Key::X, [item]{
      ...
      dialog->addButton("Yes", [item, dialog](brls::View* view) {
        GlobalObjects::getModBrowser().getModsPreseter().deleteModPreset( item->getLabel() );
        dialog->close();
...
void TabModPresets::updatePresetItems() {
  ...
  auto presetsList = GlobalObjects::getModBrowser().getModsPreseter().getPresetsList();
  ...
  for( auto& preset : presetsList ){
    ...
    auto modsList = GlobalObjects::getModBrowser().getModsPreseter().getModsList(preset );
