...
#define USB_BULK_MAX_URB_SIZE (256 * 1024)
#define USB_CANCEL_TIMEOUT_NS (250ULL * 1000ULL * 1000ULL)
    ...
    if (g_usbCommsInitialized) {
        ...
        if (R_SUCCEEDED(rc)) {
            ...
            if (hosversionAtLeast(5,0,0)) {
                ...
                // Send product
                if (R_SUCCEEDED(rc)) rc = usbDsAddUsbStringDescriptor(&iProduct, "Simple Mod Manager MTP");
                // Send serial number
...
static Result _usbCommsTransfer(usbCommsEndpoint *ep, UsbDirection dir, const void* buffer, size_t size, u64 timeout, size_t *transferredSize)
{
    ...
    while(size)
    {
        ...
        else
        {
            ...
            chunksize = size > USB_BULK_MAX_URB_SIZE ? USB_BULK_MAX_URB_SIZE : size;
            transfer_type = 1;
        }
        ...
        if (R_FAILED(rc))
        {
            ...
            Result cancel_rc = eventWait(&ep->endpoint->CompletionEvent, USB_CANCEL_TIMEOUT_NS);
            if (R_SUCCEEDED(cancel_rc)) {
...
size_t usbTransfer(u32 interface, u32 endpoint, UsbDirection dir, void* buffer, size_t size, u64 timeout)
{
    ...
    if (!initialized) return (size_t)-1;
