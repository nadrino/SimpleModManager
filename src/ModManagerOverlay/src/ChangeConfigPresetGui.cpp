...
ChangeConfigPresetGui::ChangeConfigPresetGui(){
  ...
  _answers_ = GlobalObjects::gGameBrowser.getConfigHandler().get_presets_list();
  ...
  for(int i_answer = 0 ; i_answer < int(_answers_.size()) ; i_answer++){
    ...
    clickableListItem->setClickListener([selected_answer, this](u64 keys) {
      if (keys & HidNpadButton_A) {
        GlobalObjects::gGameBrowser.change_config_preset(selected_answer);
        tsl::goBack();
        return true;
      }
      return false;
    ...
    std::string install_path_display = "install-mods-base-folder: "+ GlobalObjects::gGameBrowser.getConfigHandler().get_parameter(
