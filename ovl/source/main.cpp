...
std::string __version_str__ = "v" + toolbox::get_app_version();
...
public:
  ...
  tsl::elm::Element* createUI() override {
    ...
    _frame_ = new tsl::elm::OverlayFrame("SimpleModManager", __version_str__);
  ...
  void fill_item_list(){
    ...
    for(int i_folder = 0 ; i_folder < int(mods_list.size()) ; i_folder++){
      ...
      if(mod_fraction == -1){
        mod_fraction = 0;
        _list_->addItem(new tsl::elm::CustomDrawer([mod_fraction](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
    ...
    _list_->addItem(new tsl::elm::CategoryHeader("Mods Preset"));
...
public:
  ...
  void fill_item_list(){
    ...
    // List Items
    _list_->addItem(new tsl::elm::CategoryHeader("Folder : " + __mod_browser__.get_current_directory()));
...
public:
  ...
  void initServices() override {
    ...
    fsdevMountSdmc();
    toolbox::setFileSystemBuffer(fsdevGetDeviceFileSystem("sdmc"));
